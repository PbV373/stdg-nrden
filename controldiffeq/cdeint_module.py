...
class VectorFieldGDE(torch.nn.Module):
    ...
    def __call__(self, t, z):
        ...
        # match channel dims
        if vector_field_f.shape[1] != control_gradient.shape[1]:
            # trim wider tensor
            if vector_field_f.shape[1] > control_gradient.shape[1]:
                vector_field_f = vector_field_f[:, :control_gradient.shape[1], :]
            else:
                # padding would go here
                raise ValueError("The dimension of vector_field_f is smaller than control_gradient.")

        dh = (vector_field_f @ control_gradient.unsqueeze(-1)).squeeze(-1)
        # temporal branch omitted in this class
        return dh
...
class VectorFieldGDE_dev(torch.nn.Module):
    ...
    def __call__(self, t, hz):
        ...
        # match channel dims
        if vector_field_g.shape[1] != vector_field_f.shape[1]:
            # crop to common width
            min_size = min(vector_field_g.shape[1], vector_field_f.shape[1])
            vector_field_g = vector_field_g[:, :min_size, :, :]
            vector_field_f = vector_field_f[:, :min_size, :, :]

        try:
            vector_field_fg = torch.matmul(vector_field_g, vector_field_f)
        except RuntimeError as e:
            print(f"matmul error: {e}")
            print(f"vector_field_g shape: {vector_field_g.shape}")
            print(f"vector_field_f shape: {vector_field_f.shape}")
            raise
