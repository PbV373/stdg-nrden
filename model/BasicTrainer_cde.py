...
class Trainer(object):
    ...
    def train_epoch(self, epoch):
        if epoch <= self.args.warmup_epochs:
            lr_scale = min(1., float(epoch + 1) / float(self.args.warmup_epochs))
            for pg in self.optimizer.param_groups:
                pg['lr'] = lr_scale * self.args.lr_init
        self.model.train()
        total_loss = 0
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            *train_coeffs, target = batch
            ...
            # save sample adjacency occasionally
            if self.vector_field_g is not None and ((epoch == 1 and batch_idx == 0) or (epoch % 10 == 0 and batch_idx == 0)):
                try:
                    adj_matrix = self.vector_field_g.get_adjacency_matrix()
                    if batch_idx == 0:
                        # validate type/shape
                        if isinstance(adj_matrix, torch.Tensor):
                            adj_matrix_np = adj_matrix.cpu().detach().numpy()
                        elif isinstance(adj_matrix, np.ndarray):
                            adj_matrix_np = adj_matrix
                        else:
                            raise TypeError(f"Adjacency must be Tensor or ndarray, got {type(adj_matrix)}")

                        if adj_matrix_np.ndim != 2:
                            raise ValueError(
                                f"Adjacency must be 2D, got ndim={adj_matrix_np.ndim}, shape={adj_matrix_np.shape}")

                        # log shape (expect node axes, not time)
                        self.logger.info(f'Adjacency shape: {adj_matrix_np.shape} (expect [num_nodes, num_nodes])')

                        # save .npy
                        adj_matrix_path = os.path.join(self.args.log_dir, f'adj_matrix_epoch_{epoch}_sample.npy')
                        np.save(adj_matrix_path, adj_matrix_np)
                        self.logger.info(f'Saved adjacency sample: {adj_matrix_path}')

                # error taxonomy
                except TypeError as e:
                    self.logger.error(f'get_adjacency_matrix TypeError: {e}')
                except ValueError as e:
                    self.logger.error(f'get_adjacency_matrix ValueError: {e}')
                except IOError as e:
                    self.logger.error(f'save adjacency IOError: {e}')
                except Exception as e:
                    self.logger.error(f'save adjacency unexpected: {e}')
