...
def init_seed(seed, cudnn_benchmark=True):
    """
    cudnn_benchmark=True: nondeterministic but often faster per epoch (typical for training).
    cudnn_benchmark=False: enables cuDNN deterministic mode (slower, more reproducible).
    """
    random.seed(seed)
    np.random.seed(seed)
