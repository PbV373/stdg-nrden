...
def MAE_torch(pred, true, mask_value=None):
    ...
    return torch.round(torch.mean(torch.abs(true-pred)) * 10000) / 10000
...
def MSE_torch(pred, true, mask_value=None):
    if mask_value != None:
        ...
        pred = torch.masked_select(pred, mask)
        true = torch.masked_select(true, mask)
    return torch.round(torch.mean((pred - true) ** 2) * 10000) / 10000
...
def RMSE_torch(pred, true, mask_value=None):
    if mask_value != None:
        ...
        pred = torch.masked_select(pred, mask)
        true = torch.masked_select(true, mask)
    return torch.round(torch.sqrt(torch.mean((pred - true) ** 2)) * 10000) / 10000
...
def RRSE_torch(pred, true, mask_value=None):
    if mask_value != None:
        ...
        pred = torch.masked_select(pred, mask)
        true = torch.masked_select(true, mask)
    result = torch.sqrt(torch.sum((pred - true) ** 2)) / torch.sqrt(torch.sum((pred - true.mean()) ** 2))
...
def MAPE_torch(pred, true, mask_value=None):
    if mask_value != None:
        ...
        pred = torch.masked_select(pred, mask)
        true = torch.masked_select(true, mask)
    result = torch.mean(torch.abs(torch.div((true - pred), true)))
    return torch.round(result * 10000) / 10000
...
def PNBI_torch(pred, true, mask_value=None):
    ...
    return torch.round(indicator.mean() * 10000) / 10000
...
def oPNBI_torch(pred, true, mask_value=None):
    ...
    return torch.round(bias.mean() * 10000) / 10000
...
def MARE_torch(pred, true, mask_value=None):
    if mask_value != None:
        ...
        pred = torch.masked_select(pred, mask)
        true = torch.masked_select(true, mask)
    result = torch.div(torch.sum(torch.abs((true - pred))), torch.sum(true))
    return torch.round(result * 10000) / 10000
...
def SMAPE_torch(pred, true, mask_value=None):
    if mask_value != None:
        ...
        pred = torch.masked_select(pred, mask)
        true = torch.masked_select(true, mask)
    result = torch.mean(torch.abs(true-pred)/(torch.abs(true)+torch.abs(pred)))
    return torch.round(result * 10000) / 10000
...
def MAE_np(pred, true, mask_value=None):
    ...
    return np.round(MAE * 10000) / 10000
...
def RMSE_np(pred, true, mask_value=None):
    ...
    return np.round(RMSE * 10000) / 10000
...
def RRSE_np(pred, true, mask_value=None):
    if mask_value != None:
        ...
        pred = pred[mask]
    mean = true.mean()
    result = np.divide(np.sqrt(np.sum((pred-true) ** 2)), np.sqrt(np.sum((true-mean) ** 2)))
...
def MAPE_np(pred, true, mask_value=None):
    if mask_value != None:
        ...
        true = true[mask]
        pred = pred[mask]
    result = np.mean(np.absolute(np.divide((true - pred), true)))
    return np.round(result * 10000) / 10000
...
def PNBI_np(pred, true, mask_value=None):
    ...
    return np.round(indicator.mean() * 10000) / 10000
...
def oPNBI_np(pred, true, mask_value=None):
    if mask_value != None:
        ...
        pred = pred[mask]
    bias = (true + pred) / (2 * true)
    return np.round(bias.mean() * 10000) / 10000
...
def MARE_np(pred, true, mask_value=None):
    if mask_value != None:
        ...
        true = true[mask]
        pred = pred[mask]
    result = np.divide(np.sum(np.absolute((true - pred))), np.sum(true))
    return np.round(result * 10000) / 10000
